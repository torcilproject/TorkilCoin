...
*Setup instructions for a gitian build of TorkilCoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the TorkilCoin
...
- [Building TorkilCoin](#building-torkilcoin)
...
Clone the git repositories for torkilcoin and gitian and then checkout the torkilcoin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/torkilcoin-crypto/torkilcoin.git
cd torkilcoin
...
Follow the instructions in [doc/release-process.md](release-process.md) in the torkilcoin repository
...
To build TorkilCoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the torkilcoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/torkilcoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/torkilcoin-crypto/torkilcoin
...
For example:
```bash
URL=https://github.com/crowning-/torkilcoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit torkilcoin=${COMMIT} --url torkilcoin=${URL} ../TorkilCoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit torkilcoin=${COMMIT} --url torkilcoin=${URL} ../TorkilCoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit torkilcoin=${COMMIT} --url torkilcoin=${URL} ../TorkilCoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/torkilcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/torkilcoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/torkilcoin-build.assert
```
...
[torkilcoin/gitian.sigs](https://github.com/torkilcoin-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
