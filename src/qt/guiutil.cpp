...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a TorkilCoin address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no TorkilCoin: URI
...
{
    // Convert torkilcoin:// to torkilcoin:
    //
    //    Cannot handle this later, because torkilcoin:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open torkilcoin.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "TorkilCoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for TorkilCoin.lnk
...
{
    return GetAutostartDir() / "torkilcoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a torkilcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=TorkilCoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the torkilcoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add torkilcoin app to startup item list
