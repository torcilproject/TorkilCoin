...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeTorkilcoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeTorkilcoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeTorkilcoinAmount = strAnonymizeTorkilcoinAmount.remove(strAnonymizeTorkilcoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeTorkilcoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeTorkilcoinAmount * COIN) nMaxToAnonymize = nAnonymizeTorkilcoinAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeTorkilcoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeTorkilcoinAmount));
        strAnonymizeTorkilcoinAmount = strAnonymizeTorkilcoinAmount.remove(strAnonymizeTorkilcoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeTorkilcoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
...
void OverviewPage::toggleObfuscation()
{
    ...
    if (!fEnableObfuscation) {
        ...
        if (nAnonymizeTorkilcoinAmount == 0) {
